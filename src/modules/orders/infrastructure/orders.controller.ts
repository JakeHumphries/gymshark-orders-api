...
import { deletePackSize } from '../usecases/delete-pack-size/delete-pack-size.usecase';
...
class OrdersController {
  ...
  deletePackSize(req: Request, res: Response, next: NextFunction) {
    try {
      if (!('packSize' in req.body)) {
        throw new InvalidBodyError(JSON.stringify(req.body));
      }
      const { packSize } = req.body;
      deletePackSize.execute(Number(packSize));
      res.json(`Pack: ${packSize} has been deleted from the list of pack sizes.`);
    } catch (err) {
      next(err);
    }
  }
